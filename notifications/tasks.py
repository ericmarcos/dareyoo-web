...
@shared_task(name='send_invite_notification')
def send_invite_notifications(user_id=None, bet_id=None, **kwargs):
    if settings.GENERATE_NOTIFICATIONS:
        b = Bet.objects.get(pk=bet_id)
        u = DareyooUser.objects.get(id=user_id)
        n = NotificationFactory.bet_received(u, b)
        n.save()
        n.send_notification_email()