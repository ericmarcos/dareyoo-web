...
run(['$http', '$cookies', '$rootScope', '$state', '$stateParams', '$timeout', 'config', function run($http, $cookies, $rootScope, $state, $stateParams, $timeout, config) {
    ...
    $rootScope.getMe = function() {
      $http.get($window.location.origin + "/api/v1/me/").success(function(response) {
          $rootScope.user = response;
      });
      $timeout($rootScope.getMe, 5000);
    };
    $rootScope.getMyFollowers = function() {
      $http.get($window.location.origin + "/api/v1/me/followers").success(function(response) {
    ...
    $rootScope.getNotifications = function() {
      $http.get($window.location.origin + "/api/v1/notifications/").success(function(response) {
        var not = [];
    ...
    $rootScope.notificationViewed = function(note) {
      if(note.is_new) {
        note.is_new = false;
        $http.post($window.location.origin + '/api/v1/notifications/' + note.id + '/mark_as_viewed/');
      }
    };
    ...
    $rootScope.notificationClick = function(note) {
      if(!note.readed) {
        note.readed = true;
        $http.post($window.location.origin + '/api/v1/notifications/' + note.id + '/mark_as_readed/');
      }
    }
