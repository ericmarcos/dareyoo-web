...
angular.module('dareyoo.controllers', []).
  ...
  .controller('BetCtrl', ['$scope', '$rootScope', '$http', '$stateParams', function($scope, $rootScope, $http, $stateParams) {
    ...
    $scope.acceptBet = function() {
      $http.post(document.location.origin + "/api/v1/bets/" + $scope.bet.slug + "/accept_bet/").success(function(response) {
        $scope.getBet($stateParams.betId);
      }).error($scope.betAPIError);
    };
    $scope.postBid = function(title, amount) {
      $http.post(document.location.origin + "/api/v1/bets/" + $scope.bet.slug + "/bids/", {title: title, amount: amount}).success(function(response) {
        $scope.getBet($stateParams.betId);
    ...
    $scope.acceptBid = function(bidId) {
      $http.post(document.location.origin + "/api/v1/bets/" + $scope.bet.slug + "/accept_bid/", {bid_id: bidId}).success(function(response) {
        $scope.getBet($stateParams.betId);
      }).error($scope.betAPIError);
    };
    ...
    $scope.removeBid = function(bidId) {
      $http.post(document.location.origin + "/api/v1/bets/" + $scope.bet.slug + "/remove_bid/", {bid_id: bidId}).success(function(response) {
        $scope.getBet($stateParams.betId);
      }).error($scope.betAPIError);
    };
    $scope.resolveBet = function(claim) { /* {1: Bet author won, 2: Bid author won, 3: null} */
      $http.post(document.location.origin + "/api/v1/bets/" + $scope.bet.slug + "/resolve/", {claim: claim, claim_message: $scope.claims.resolvingClaim}).success(function(response) {
        $scope.getBet($stateParams.betId);
      }).error($scope.betAPIError);
    };
    $scope.resolveLottery = function(bidId, claim) { /* {1: Bet author won, 2: Bid author won, 3: null} */
      if(claim == 3) {
        ...
        $http.post(document.location.origin + "/api/v1/bets/" + $scope.bet.slug + "/resolve/", {claim_lottery_winner: bidId, claim_message: $scope.claims.resolvingClaim}).success(function(response) {
          $scope.getBet($stateParams.betId);
        }).error($scope.betAPIError);
      }
    };
    ...
    $scope.arbitrateBet = function(claim) { /* {1: Bet author won, 2: Bid author won, 3: null} */
      $http.post(document.location.origin + "/api/v1/bets/" + $scope.bet.slug + "/arbitrate/", {claim: claim, claim_message: $scope.claims.arbitratingClaim}).success(function(response) {
        $scope.getBet($stateParams.betId);
      }).error($scope.betAPIError);
    };
    $scope.arbitrateLottery = function(claim_lottery_winner, claim) { /* {1: Bet author won, 2: Bid author won, 3: null} */
      if(claim == 3) {
        ...
        $http.post(document.location.origin + "/api/v1/bets/" + $scope.bet.slug + "/arbitrate/", {claim_lottery_winner: claim_lottery_winner, claim_message: $scope.claims.arbitratingClaim}).success(function(response) {
          $scope.getBet($stateParams.betId);
        }).error($scope.betAPIError);
      }
    };
  ...
  .controller('NewBetCtrl', ['$scope', '$http', '$timeout', '$rootScope', function($scope, $http, $timeout, $rootScope) {
    ...
    $scope.postNewBet = function() {
      ...
      .success(function(response, status, headers, config) {
        ...
        $scope.invites = [];
        $rootScope.$broadcast('rebuildTwitter');
        $rootScope.$state.go("main.bet", {betId:$scope.new_bet.slug});
      })
